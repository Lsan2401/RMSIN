...
def evaluate(model, data_loader, bert_model, epoch):
    ...
    for n_eval_iou in range(len(eval_seg_iou_list)):
        ...
        writer.add_scalar('val_precision@%s' % str(eval_seg_iou_list[n_eval_iou]), seg_correct[n_eval_iou] * 100. / seg_total, epoch)

    results_str += '    overall IoU = %.2f\n' % (cum_I * 100. / cum_U)
    print(results_str)

    writer.add_scalar('val_loss', total_loss / total_its, epoch)
    writer.add_scalar('val_MeanIoU', mIoU, epoch)
    writer.add_scalar('val_overallIoU', cum_I * 100. / cum_U, epoch)
...
def train_one_epoch(model, criterion, optimizer, data_loader, lr_scheduler, epoch, print_freq,
    ...
    for data in metric_logger.log_every(data_loader, print_freq, header):
        ...
        # scaler.update()
          # set_to_none=True is only available in pytorch 1.6+


        loss = criterion(output, target)#/iter_size
        loss.backward()
        # if (total_its+1) % iter_size == 0:
        #     optimizer.step()
        #     lr_scheduler.step()
        #     optimizer.zero_grad()
        optimizer.step()
    ...
    # writer.add_scalar('train_loss', train_loss / total_its, epoch)
        ...
        params_to_optimize = [
            ...
            {"params": reduce(operator.concat,
                              [[p for p in single_model.text_encoder.encoder.layer[i].parameters()
                                if p.requires_grad] for i in range(10)])},
            # {"params": [p for p in layer.fusion.parameters()] for layer in single_model.backbone.layers},
        ]
