...
def main(args):
    ...
    # Randomly select 100 indices from your dataset
    selected_indices = random.sample(range(len(dataset_test)), 100)

    # Get the selected data items
    selected_data = [dataset_test[i] for i in selected_indices]

    # test_sampler = torch.utils.data.SequentialSampler(selected_data)
